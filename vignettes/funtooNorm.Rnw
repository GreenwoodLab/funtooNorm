...
The probe names should index the rows, and the sample names should index the columns.
 {\color{red} These can be extracted from the Illumina IDAT files using Genome Studio  (http://www.illumina.com/applications/microarrays/microarray-software/genomestudio.html) by ..., or extracted directly from the IDAT files with Bioconductor packages such as .... }.
...
{\color{red} This information can also be extracted from the IDAT files using Genome Studio, by .... , or ....}
...
We have provided a small data set containing $N=93$ individuals and 1000 {\color{red} I don't think that 1000 is not enough for testing. Try 20,000} probes to demonstrate the usage of the package. 
