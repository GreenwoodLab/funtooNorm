...
The probe names should index the rows, and the sample names should index the columns.
 {\color{red} These can be extracted from the Illumina IDAT files using Genome Studio  (http://www.illumina.com/applications/microarrays/microarray-software/genomestudio.html) by
Extracting Signal_A and Signal_B from the Sample Methylation Profile or extracted directly from the IDAT files with Bioconductor packages such as \textbf{illuminaio}}, \cite{Smith2013}.
...
{\color{red} This information can also be extracted from the IDAT files using Genome Studio, by 
Extracting Signal_Grn and Signal_Red from the Control Probe Profile or extracted directly from the IDAT files with Bioconductor packages such as \textbf{illuminaio} and \textbf{IlluminaDataTestFiles}
}
...
There must be at least two different cell or tissue types in the data {\color{red} or the program will terminate with a warning}.
\end{itemize}
...
We have provided a small data set containing $N=93$ individuals and 20000 probes to demonstrate the usage of the package. 
...
<<>>= 
    ...
    funtoonormout <- funtoonorm(sigA=sigAsample, sigB=sigBsample, Annot=NULL, 
                      ...
                      ncmp=4, save.quant=TRUE, save.loess=TRUE, apply.loess=TRUE, logit.quant=TRUE, validate=FALSE)
...
\caption{Cross-validated errors across percentiles of the signal distributions for different numbers of PLS components.  Left: signal A; Right: signal B;  Top: probe type I red; Middle: probe type I green; Bottom: probe type II.  } \label{val}
\end{figure}
