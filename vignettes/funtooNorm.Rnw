...
The \textbf{funtooNorm} package provides functions for normalization of Illumina Infinium Human Methylation 450 BeadChip (Illumina 450K) data when there are samples from multiple tissues or cell types. 
...
\begin{itemize}
\item \textbf{sigA, sigB:} Two matrices containing the signal A and signal B information from the IDAT files.
...
These can be obtained from the Illumina IDAT files using Genome Studio  (http://www.illumina.com/applications/microarrays/microarray-software/genomestudio.html) by extracting Signal_A and Signal_B from the Sample Methylation Profile, or these can be obtained directly from the IDAT files with Bioconductor packages such as \textbf{illuminaio} and \textbf{IlluminaDataTestFiles}, \cite{Smith2013}. \textbf{illuminaio} provides function readIDAT to extract sygnal from IDAT files, control probes signal could be separated using probe summaries provided in file 4343238080_A_ProbeSummary.txt.gz of \textbf{IlluminaDataTestFiles}.
...
A default annotation table is provided if not supplied.
...
\emph{save.quant} implies that the quantiles should be saved;  \emph{save.loess} means that loess fits to the curves should be saved; \emph{apply.loess} means that the normalization itself should be applied to all the data based on the loess fits.
...
To access the performance of normalization function one can use a measure of intra-replicate differences \emph{M}, described in \cite{funtooNorm}. We provide a function \emph{agreement} implementing this measure. It takes as arguments a matrix of beta values and a vector of patient ID's. For the function to work some elements of patient's vector, obviously, should be identical. The returned value of \emph{M} is expected to be similar for the data before and after normalization:
%\begin{Verbatim}
<<>>= 
agreement(funtoonormout[[1]], patientid) # M for data before the normalization
agreement(funtoonormout[[2]], patientid) # M for data after normalization
@
%\end{Verbatim} 
